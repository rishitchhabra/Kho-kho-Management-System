...
async function createMatch(matchData) {
    try {
        // Get current max order and match_number
        ...
        const maxOrder = existingMatches.length > 0 
            ...
            : 0;
        // Match number is permanent and sequential (never reused)
        const maxMatchNumber = existingMatches.length > 0 
            ? Math.max(...existingMatches.map(m => m.match_number || m.match_order || 0)) 
            : 0;
        ...
        return { success: true, data, match: data?.[0] };
    } catch (error) {
...
async function createMatchesForPool(poolId, teamIds, teamType) {
    try {
        ...
        let maxOrder = existingMatches.length > 0 
            ...
            : 0;
        // Match number is permanent and sequential (never reused)
        let maxMatchNumber = existingMatches.length > 0 
            ? Math.max(...existingMatches.map(m => m.match_number || m.match_order || 0)) 
            : 0;
...
/**
 * Update match order and match number
 * @param {number} matchId - Match ID
 * @param {number} newOrder - New display order number
 * @param {number} newMatchNumber - New permanent match number (optional, uses newOrder if not provided)
 * @returns {Promise<boolean>} Success status
 */
async function updateMatchOrder(matchId, newOrder, newMatchNumber = null) {
    try {
        // Update both match_order and match_number
        const { error } = await supabase
            .from('matches')
            .update({ 
                match_order: newOrder,
                match_number: newMatchNumber !== null ? newMatchNumber : newOrder
            })
            .eq('id', matchId);
